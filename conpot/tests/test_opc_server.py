...
from conpot.tests.helpers import opc_client
from conpot.protocols.opc.opc_server import OPCServer
...
class TestBase(unittest.TestCase):
    def setUp(self):
        ...
        self.opc_server = OPCServer('conpot/templates/default/opc/opc.xml', 'none', args)
        self.server_greenlet = gevent.spawn(self.opc_server.start, self.host, 0)
        gevent.sleep(1)
        self.port = self.opc_server.get_port()

    def tearDown(self):
        shutil.rmtree(self.tmp_dir)

    def test_opc_get(self):
        """
        Objective: Test if we can get data via opc_get
        """
        client = opc_client.OPCClient(self.host, self.port)
    ...
    def test_opc_set(self):
        """
        Objective: Test if we can set data via opc_set
        """
        client = opc_client.OPCClient(self.host, self.port)
        # syslocation
