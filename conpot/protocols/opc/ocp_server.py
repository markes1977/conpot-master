...
from conpot.protocols.opc.command_responder import CommandResponder
from conpot.protocols.opc.build_pyopc_mib_wrapper import find_mibs, compile_mib
...
class OPCServer(object):
    def __init__(self, template, template_directory, args):
        ...
        if args.mibpaths:
            ...
            self.compiled_mibs = [os.path.join(template_directory, 'opc', 'mibs_compiled')]

        if args.raw_mib:
            self.raw_mibs = args.raw_mib
        else:
            self.raw_mibs = [os.path.join(template_directory, 'opc', 'mibs_raw')]

    def xml_general_config(self, dom):
        opc_config = dom.xpath('//opc/config/*')
        if opc_config:
            for entity in opc_config:
    ...
    def config_sanitize_tarpit(self, value):
        ...
        if value is not None:
            ...
            try:
                ...
                logger.error("OPC invalid tarpit value: '%s'. Assuming no latency.", value)
    ...
    def config_sanitize_threshold(self, value):
        ...
        if value is not None:
            ...
            try:
                ...
                logger.error("OPC invalid evasion threshold: '%s'. Assuming no DoS evasion.", value)
                # first value is invalid, ignore the whole setting.
                return '0;0'
    ...
    def start(self, host, port):
        ...
        logger.info('OPC server started on: %s', (host, self.get_port()))
