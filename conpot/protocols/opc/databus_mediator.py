    ...
    def get_response(self, reference_class, OID):
        if OID in self.oid_map:
            if reference_class == 'DisplayString':
                (response_class,) = builder.MibBuilder().importSymbols("OPCv2-TC", "DisplayString")
                ...
                (response_class,) = builder.MibBuilder().importSymbols("OPCv2-SMI", "Integer32")

            elif reference_class == 'Counter32':
                (response_class,) = builder.MibBuilder().importSymbols("OPCv2-SMI", "Counter32")

            elif reference_class == 'Gauge32':
                (response_class,) = builder.MibBuilder().importSymbols("OPCv2-SMI", "Gauge32")

            elif reference_class == 'TimeTicks':
                (response_class,) = builder.MibBuilder().importSymbols("OPCv2-SMI", "TimeTicks")
